...
<script type="text/javascript">
    ...
    $(function() {   	
        ...
        importExcel();
        treatSubjectTransDataGrid = $('#treatSubjectTransDataGrid').datagrid({
            url : '${path }/treatsubjectTrans/treatSubjectTransDataGrid',
            fit : true,
            striped : true,
           // rownumbers : true,
            pagination : true,
          ...
          //  idField : 'id',
            ...
            columns : [ [ {
                ...
                width : '15%',
                title : '<font size="3px">自定义项目包编码</font>',
                field : 'definesubjectCode'
    	...
    	$('#diagnosisEditForm').form({
   		 url :'${path }/treatsubjectTrans/updateTreatSubjectTrans',
   		 onSubmit : function() {
                  progressLoad();
              ...
              success : function(result) {
              	...
              	{
               	   hideDiv2();
               	treatSubjectTransDataGrid.datagrid('load', $.serializeObject($('#searchForm')));
    ...
    function deleteFun() {
       /*  if (id == undefined) {//点击右键菜单才会触发这个
   	 ...
   	 var ss = "";
     var rows = $('#treatSubjectTransDataGrid').datagrid('getSelections');
     if(rows.length<1){
     ...
     for(var i=0; i<rows.length; i++){
        ...
        parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
            if (b) {
                ...
                if (b) {
                    progressLoad();
                    $.post('${path }/treatsubjectTrans/deleteTreatsubjectTrans', {
                        id : ss
                    }, function(result) {
                        if (result.success) {
                            parent.$.messager.alert('提示', result.msg, 'info');
                            treatSubjectTransDataGrid.datagrid('reload');
                        }
                        progressClose();
                    }, 'JSON');
                }
    ...
    function editDiagnosisFun() {
        ...
        if(rows.length>1){
    	 ...
    	 $.post('${path }/treatsubjectTrans/editPage', {
    		 Id : id
           }, function(result) {
          	 showDiv2(); 
               $('#id').val(result.id);
               $('#subjectCodeEdit').val(result.subjectCode);
               $('#definesubjectCodeEdit').val(result.definesubjectCode);
               $('#subjectNameEdit').val(result.subjectName);
               $('#messageEdit').val(result.message);
           }, 'JSON');
    ...
    function cleanFun() {
    	$('#searchForm')[0].reset();
        treatSubjectTransDataGrid.datagrid('load', {});
    }
    ...
    function importExcel()
    {
    	var imgobjs=document.getElementById("daoru");
       	...
       	$(imgobjs).fileUpload(
    		{
    			uploadURL : '${path}/treatsubjectTrans/importExcel',
    ...
    function exportTreatsubjectTrans()
    {
    	...
    	location.href='${path}/treatsubjectTrans/exportTreatsubjectTrans';
    	
    }
    function exportTreatsubjectTransHistory()
    {
    	parent.$.messager.alert({
            title : '提示',
            msg : '正在导出数据... 请耐心等待！'
    	...
    	location.href='${path}/treatsubjectTrans/exportTreatsubjectTransHistory';
    }
    function exportTemp()
    {
    	location.href='${path}/treatsubjectTrans/exportTemp';
    }
</script>
</head>
<body>
...
<jsp:include page="../include/tophead.jsp" />
	...
	<div class="body-width fs_18 bold header-color">治疗项目转化</div>
  ...
  <div  style="width: 80%;float: left">
    <table class="ys-table" style="width: 95%">
        ...
        <tr>
            ...
            <td>
               <input name="subjectCode"  id="" style="width:120px;height: 30px;" />
            </td>
            ...
            <td>
               <input name="subjectName"  id="" style="width:120px;height: 30px;" />
            </td>
           
      
            <td class="td-width">自定义项目包编码</td>
            <td>
             <input name="definesubjectCode"  id="" style="width:120px;height: 30px;" />
   ...
   <table id="btn-table" class="float margin-top10" style="width: 100%;">
    <tr>
        ...
        <td class="right" style="width: 80px">
            <button class="btn-ok fs_16" onclick="exportTreatsubjectTrans()">导出明细</button>
        </td>
        <td class="right" style="width: 96px">
            <button class="btn-ok fs_16" style="width: 91px;" onclick="exportTreatsubjectTransHistory()">查看历史记录</button>
        </td>
	...
	<div class="body-width   margin-top15" style="height: 340px; overflow-x: auto;">
		<table class="sf-msg-table border-val" id="treatSubjectTransDataGrid"style="width: 100%">
    ...
    <div class="DialogDiv1" style="display: none">
        <div class="pop-up bg-color1">
            <span class="float pop-up-span width-color dis-block">治疗项目转化新增</span>
        </div>
        ...
        <div class="body-width border-val" style="height: 250px;margin-top: 20px !important;width: 94%;margin-left: 3%;">
            <table class="bz-zd-table">
                ...
                <tr>
                    <td>
                        <span class="bz-zd-table-span">*</span>医保项目编码
                    </td>
                    <td>
                      <input name="subjectCode"  id="subjectCodeAdd" type="text" class="easyui-validatebox" validateOnCreate="false"
                        validType="account[1,40]"  data-options="required:true" >
                    </td>
                    <td>
                        <span class="bz-zd-table-span">*</span>自定义项目包编码
                    </td>
                    <td >
                       <input name="definesubjectCode" type="text"  id="definesubjectCodeAdd" class="easyui-validatebox" validateOnCreate="false"
                        validType="account[1,5]"   data-options="required:true" >
                    </td>
                  
                      <td>
                        <span class="bz-zd-table-span">*</span>医保项目名称
                    </td>
                    <td>
                      <input name="subjectName"  id="subject_nameAdd"  type="text" validateOnCreate="false"
                 ...
                 <tr >
                      <td >
                        <span class="bz-zd-table-span"></span>提示信息
                    </td>
                    <td colspan="5">
                      <textarea   style="margin-top: -0px;"  style="height: 80px" maxlength="50"  id="messageAdd" name="message" cols="160" rows="5"></textarea>
                    </td>
                 </tr>            
                </tbody>
            </table>
    ...
    <div class="DialogDiv2" style="display: none">
        <div class="pop-up bg-color1">
            <span class="float pop-up-span width-color dis-block">治疗项目转化修改</span>
        </div>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      ...
                      <input name="subjectCode"  id="subjectCodeEdit" type="text" class="easyui-validatebox" validateOnCreate="false"
                        validType="account[1,40]"  data-options="required:true" >
                    </td>
                    <td>
                        <span class="bz-zd-table-span">*</span>自定义项目包编码
                    ...
                    <td >
                       <input name="definesubjectCode" type="text"  id="definesubjectCodeEdit" class="easyui-validatebox" validateOnCreate="false"
                        validType="account[1,5]"  data-options="required:true" >
                    </td>
                    ...
                    <td>
                      <input name="subjectName"  id="subjectNameEdit"  type="text" validateOnCreate="false"
                       class="easyui-validatebox"  validType="length[1,50]"   data-options="required:true">
                    </td>
                </tr>
                 ...
                 <tr >
                    ...
                    <td colspan="5">
                      <textarea   style="margin-top: -0px;"  style="height: 80px" maxlength="50"  id="messageEdit" name="message" cols="160" rows="5"></textarea>
                    </td>
                 </tr>
