...
<script type="text/javascript">
    ...
    $(function() {
        ...
        importExcel();
        diagnosisCatalogDataGrid = $('#diagnosisCatalogDataGrid').datagrid({
            url : '${path }/diagnosisCatalog/diagnosisCatalogDataGrid',
            fit : true,
            striped : true,
           // rownumbers : true,
            pagination : true,
    	...
    	$('#diagnosisEditForm').form({
   		 url :'${path }/diagnosisCatalog/updateDiagnosisCatalog',
   		 onSubmit : function() {
                  progressLoad();
              ...
              success : function(result) {
              	...
              	{
               	   hideDiv2();
               	diagnosisCatalogDataGrid.datagrid('load', $.serializeObject($('#searchForm')));
    ...
    function deleteFun() {
       /*  if (id == undefined) {//点击右键菜单才会触发这个
   	 ...
   	 var ss = "";
     var rows = $('#diagnosisCatalogDataGrid').datagrid('getSelections');
     if(rows.length<1){
     ...
     for(var i=0; i<rows.length; i++){
        ...
        parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
            if (b) {
                ...
                if (b) {
                    progressLoad();
                    $.post('${path }/diagnosisCatalog/deleteDiagnosticCatalog', {
                        id : ss
                    }, function(result) {
                        if (result.success) {
                            parent.$.messager.alert('提示', result.msg, 'info');
                            diagnosisCatalogDataGrid.datagrid('reload');
                        }
                        progressClose();
                    }, 'JSON');
                }
    ...
    function editDiagnosisFun() {
        ...
        if(rows.length>1){
    	 ...
    	 $.post('${path }/diagnosisCatalog/editPage', {
    		 Id : id
           }, function(result) {
          	 showDiv2(); 
               $('#id').val(result.id);
    ...
    function cleanFun() {        
        $('#searchForm')[0].reset();
        diagnosisCatalogDataGrid.datagrid('load', {});
    }
    ...
    function importExcel()
    {
    	var imgobjs=document.getElementById("daoru");
       	...
       	$(imgobjs).fileUpload(
    		{
    			uploadURL : '${path}/diagnosisCatalog/importExcel',
    ...
    function exportDiagnosisCatalog()
    {
    	...
    	location.href='${path}/diagnosisCatalog/exportDiagnosisCatalog';
    	
    }
    function exportDiagnosisCatalogHistory()
    {
    	parent.$.messager.alert({
            title : '提示',
            msg : '正在导出数据... 请耐心等待！'
    	...
    	location.href='${path}/diagnosisCatalog/exportDiagnosisCatalogHistory';
    }
    function exportTemp()
    {
    	location.href='${path}/diagnosisCatalog/exportTemp';
    }
</script>
</head>
<body>
...
<jsp:include page="../include/tophead.jsp" />
	...
	<div class="body-width fs_18 bold header-color">诊断目录管理</div>
  ...
  <div  style="width: 80%;float: left">
   ...
   <table id="btn-table" class="float margin-top10" style="width: 100%;">
    <tr>
        ...
        <td class="right" style="width: 80px">
            <button class="btn-ok fs_16" onclick="exportDiagnosisCatalog()">导出明细</button>
        </td>
        <td class="right" style="width: 96px">
            <button class="btn-ok fs_16" style="width: 91px;" onclick="exportDiagnosisCatalogHistory()">查看历史记录</button>
        </td>
    </tr>
</table>

	<div class="body-width   margin-top15" style="height:340px; overflow-x: auto;">
		<table class="sf-msg-table border-val" id="diagnosisCatalogDataGrid"style="width: 100%">
    ...
    <div class="DialogDiv1" style="display: none">
        <div class="pop-up bg-color1">
            <span class="float pop-up-span width-color dis-block">诊断目录管理新增</span>
        </div>
    ...
    <div class="DialogDiv2" style="display: none">
        <div class="pop-up bg-color1">
            <span class="float pop-up-span width-color dis-block">诊断目录管理修改</span>
        </div>
