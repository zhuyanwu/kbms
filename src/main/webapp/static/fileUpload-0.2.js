/**
 * 图得上传功能，最终返回图片在服务器存放的路径
 */
(function($) {
	$.fn.fileUpload = function(options) {
		defaults = {
			uploadURL : "about:black",
			callback : function() {
		...
		//	progressClose();
			...
			if (returnData != undefined && returnData != null) {
				if (options.callback != undefined && options.callback != null
					...
					options.callback.call(tempFun, returnData);
				}
			}
			...
			$.upload = function(element, options) {
				...
				return new $.ocupload(element, options);
			},
			...
			$.ocupload = function(element, options) {
				...
				/** Upload Iframe */
				var iframe = $(
						'<iframe ' + 'id="iframe' + id + '" ' + 'name="iframe'
								+ id + '"' + '></iframe>').css({
					display : 'none'
				});
				...
				var form = $(
						'<form ' + 'method="post" ' + 'enctype="'
								+ options.enctype + '" ' + 'action="'
								+ options.action + '" ' + 'target="iframe' + id
								+ '"' + '></form>').css({
					margin : 0,
					padding : 0
				});
						...
						.extend(
								this,
								{
									...
									submit : function() {
										...
										$.messager.alert( '提示',"文件正在上传中.....", 'warning');
//										 $.messager.alert( '提示',"文件正在上传中.....", 'warning',function(){});
//										 setTimeout(function(){
...
//											},2000);
										...
										/**
										 * add additional paramters before
										 * sending
										 */
										...
										/**
										 * Do something after we are finished
										 * uploading
										 */
										iframe
												.unbind()
												.load(
														function() {
															 ...
															 * in plain text
															 */
															var myFrame = document
																	.getElementById(iframe
																			.attr('name'));
															var response = $(
																	myFrame.contentWindow.document.body)
																	.text();
