...
<script type="text/javascript">
    $(function() {   	
        ...
        importExcel();
        tumourDiagnosisDataGrid = $('#tumourDiagnosisDataGrid').datagrid({
            url : '${path }/tumourDiagnosis/tumourDiagnosisDataGrid',
            fit : true,
            striped : true,
           // rownumbers : true,
            pagination : true,
    	...
    	$('#diagnosisEditForm').form({
   		 url :'${path }/tumourDiagnosis/updateTumourDiagnosis',
   		 onSubmit : function() {
                  progressLoad();
              ...
              success : function(result) {
              	...
              	{
               	   hideDiv2();
               	tumourDiagnosisDataGrid.datagrid('load', $.serializeObject($('#searchForm')));
    ...
    function deleteFun() {
       /*  if (id == undefined) {//点击右键菜单才会触发这个
   	 ...
   	 var ss = "";
     var rows = $('#tumourDiagnosisDataGrid').datagrid('getSelections');
     if(rows.length<1){
     ...
     for(var i=0; i<rows.length; i++){
        ...
        parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(b) {
            if (b) {
                ...
                if (b) {
                    progressLoad();
                    $.post('${path }/tumourDiagnosis/deleteTumourDiagnosis', {
                        id : ss
                    }, function(result) {
                        if (result.success) {
                            parent.$.messager.alert('提示', result.msg, 'info');
                            tumourDiagnosisDataGrid.datagrid('reload');
                        }
                        progressClose();
                    }, 'JSON');
                }
    ...
    function editDiagnosisFun() {
        ...
        if(rows.length>1){
    	 ...
    	 $.post('${path }/tumourDiagnosis/editPage', {
    		 Id : id
           }, function(result) {
          	 showDiv2(); 
               $('#id').val(result.id);
    ...
    function cleanFun() {
    	$('#searchForm')[0].reset();
        tumourDiagnosisDataGrid.datagrid('load', {});
    }
    ...
    function importExcel()
    {
    	var imgobjs=document.getElementById("daoru");
       	...
       	$(imgobjs).fileUpload(
    		{
    			uploadURL : '${path}/tumourDiagnosis/importExcel',
    ...
    function exportTumourDiagnosis()
    {
    	...
    	location.href='${path}/tumourDiagnosis/exportTumourDiagnosis';
    	
    }
    function exportTumourDiagnosisHistory()
    {
    	parent.$.messager.alert({
            title : '提示',
            msg : '正在导出数据... 请耐心等待！'
    	...
    	location.href='${path}/tumourDiagnosis/exportTumourDiagnosisHistory';
    }
    function exportTemp()
    {
    	location.href='${path}/tumourDiagnosis/exportTemp';
    }
</script>
</head>
<body>
...
<jsp:include page="../include/tophead.jsp" />
	...
	<div class="body-width fs_18 bold header-color">中药超量_肿瘤诊断</div>
   ...
   <table id="btn-table" class="float margin-top10" style="width: 100%;">
    <tr>
       ...
       <td class="right" style="width: 72px"><button class="btn-ok fs_16" id="daoru"  name="daoru"  >导入</button></td>
        ...
        <td class="right" style="width: 80px">
            <button class="btn-ok fs_16" onclick="exportTumourDiagnosis()">导出明细</button>
        </td>
        <td class="right" style="width: 96px">
            <button class="btn-ok fs_16" style="width: 91px;" onclick="exportTumourDiagnosisHistory()">查看历史记录</button>
        </td>
    </tr>
</table>

	<div class="body-width   margin-top15" style="height: 340px; overflow-x: auto;">
		<table class="sf-msg-table border-val" id="tumourDiagnosisDataGrid"style="width: 100%">
    ...
    <div class="DialogDiv1" style="display: none">
        <div class="pop-up bg-color1">
            <span class="float pop-up-span width-color dis-block">中药超量_肿瘤诊断新增</span>
        </div>
    ...
    <div class="DialogDiv2" style="display: none">
        <div class="pop-up bg-color1">
            <span class="float pop-up-span width-color dis-block">中药超量_肿瘤诊断修改</span>
        </div>
