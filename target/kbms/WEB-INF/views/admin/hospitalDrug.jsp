...
var hospitalDrugDataGrid;
$(function(){
	importExcel();
	hospitalDrugDataGrid=$('#hospitalDrugDataGrid').datagrid({
		url : '${path }/hospitalDrug/searchDrugInfo',
        fit : true,
        striped : true,
        pagination : true,
        singleSelect : false, 
	...
	$('#hospitalDrugEditForm').form({
		 url : '${path}/hospitalDrug/editDrugInfo',
		 onSubmit : function() {
              progressLoad();
	...
	$("#delHospitalDrug").click(function(){
		 
		var  hospitalDrugChecked = hospitalDrugDataGrid.datagrid('getSelections');
		 
		 if(hospitalDrugChecked.length<1)
		 {
		 ...
		 parent.$.messager.confirm('询问', '您是否要删除当前记录？', function(flag) {
			 ...
			 var id=[];
			 for(i=0;i<hospitalDrugChecked.length;i++)
			 {
			 	id[i]=hospitalDrugChecked[i].ID;
			 }
			 
			 progressLoad();
			 $.post('${path }/hospitalDrug/deleteDrugInfo', {
	            id : id
	        }, function(result) {
	            if (result.success) {
	                parent.$.messager.alert('提示', result.msg, 'info');
	                hospitalDrugDataGrid.datagrid('reload');
	            }
	            progressClose();
	        }, 'JSON');
...
function searchHospitalDrugFun() 
{
	hospitalDrugDataGrid.datagrid('load', $.serializeObject($('#searchHospitalDrugForm')));
}
function cleanProjectCatalogFun()
{
   $('#searchHospitalDrugForm')[0].reset();
   hospitalDrugDataGrid.datagrid('load', {});
}
...
function closeWin()
{
     ...
     $('#hospitalDrugEditForm')[0].reset();
     $('#id').val('');
}
function editWin()
{
	var  stepDrugChecked = hospitalDrugDataGrid.datagrid('getSelections');
	...
	if(stepDrugChecked.length>1)
	 ...
	 $.post('${path }/hospitalDrug/selectEditDrugInfo', {
...
function exportHospitalDrug()
{
	...
	location.href='${path}/hospitalDrug/exportDrugInfo';
	
}
function exportHospitalDrugHistory()
{
	parent.$.messager.alert({
        title : '提示',
        msg : '正在导出数据... 请耐心等待！'
	...
	location.href='${path}/hospitalDrug/exportDrugInfoHistory';
}
function exportTemp()
{
	location.href='${path}/hospitalDrug/exportTemp';
}
function importExcel()
{
	var imgobjs=document.getElementById("importExcel");
   	...
   	$(imgobjs).fileUpload(
		{
			uploadURL : '${path}/hospitalDrug/importExcel',
...
<div class="xRightbox">
<!--限适应症用药(违规)-->
<div class="body-width fs_18 bold header-color">1023_基础表</div>
<div class="header-div bg-color1 border-rgt border-lef">
  <form id="searchHospitalDrugForm">
    <table class="ys-table" style="width: 95%">
        ...
        <tr>
            ...
            <td>
                <input type="button" value="查询" onclick="searchHospitalDrugFun()" class="btn-ok fs_16"/> <input type="button" value="重置" onclick="cleanProjectCatalogFun()" class="btn-cancel fs_16"/>
...
<table id="btn-table" class="float margin-top10" style="width: 100%;">
    <tr>
        <td>
            ...
            <button class="btn-ok fs_16" id="delHospitalDrug">删除</button>
        </td>
        ...
        <td class="right" style="width: 80px">
            <button class="btn-ok fs_16" onclick="exportHospitalDrug()">导出明细</button>
        </td>
        <td class="right" style="width: 96px">
            <button class="btn-ok fs_16" style="width: 91px;" onclick="exportHospitalDrugHistory()">查看历史记录</button>
        </td>
    </tr>
</table>
...
<div class="body-width clear">
    <div style="height: 340px">
        <table id="hospitalDrugDataGrid" class="on-style">
            
        </table>
    </div>
    <div class="DialogDiv3" style="display: none">
        <div class="pop-up bg-color1">
            <span class="float pop-up-span width-color dis-block">1023_基础表<span id="optName"></span></span>
        </div>

        <!--table数据-->
         <form id="hospitalDrugEditForm" method="post">
